 ...
 * @param cwd
 * @param pkg
 * @returns format-demo-eslint-config/index
 * @returns format-demo-eslint-config/react
 * @returns format-demo-eslint-config/typescript/index
 * @returns format-demo-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'format-demo-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
